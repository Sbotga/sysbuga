...
class SongInfo(commands.Cog):
    ...
    async def aliases(self, interaction: discord.Interaction, song: str) -> None:
        ...
        # title_variants is every search key for the song. Whatever isn't a manual
        # alias or the title itself was generated by the matcher (romanizations etc).
        skip = {preprocess(a) for a in manual} | {preprocess(music.title)}
        auto = [
            v for v in dict.fromkeys(music.title_variants) if preprocess(v) not in skip
        ]
